'''
For Seoul Robotics Code Assignment
This Script was written with following software, packages
...
import data_visualize as Seoul_Robotics     # For Direct Visualization
...
class Lane:
    ...
    def __init__(self, _file_num):
        ...
        self.points = np.fromfile(self.bin_file, dtype=np.float32).reshape(-1, 5)   # (x,y,z,intensity)

        self.points_xyzi = copy.deepcopy(self.points[:, :4])    # Keep origin

        self.scale_factor = 10      # Scale up Intensity

        self.num_lanes = 2          # Left, Right Lane >> 2 Lanes

        self.bin_hist_num = 30      # Number of bins of histogram

        self.ROI_hist_min = 7.0     # ROI(Region of Interest) for histogram

        self.ROI_hist_max = 20.0    # ROI(Region of Interest) for histogram

        self.ROI_Det_min = 10.0     # ROI(Region of Interest) for Sliding Window

        self.ROI_Det_max = 60.0     # ROI(Region of Interest) for Sliding Window

        self.bin = 30               # Number for Sliding Window Bin

        self.order = 3              # Order of Polynomial fitting

        self.offset = 0.35          # y size of Sliding Window Bin

        self.data_folder = "data_folder"        # LiDAR Data bin file Folder

        self.lane_folder = "lane_folder"        # Lane Coefficient of Fitted Lane Folder
    ...
    def Pipeline(self):
        ...
        road_plane, road_plane_idx = self.find_road_plane(self.points_xyzi)

        line = self.intensity_thresholding(road_plane, threshold=30)
        ...
        show_detected_left_right_lane = True
        ...
        ############# if you want to use direct data visualization, choose True #############

        direct_use_data_visualize_py = True

        self.data_visualization(direct_use=direct_use_data_visualize_py, txt_saved=is_save)

        
    def data_visualization(self, direct_use, txt_saved):
        '''
        Data Visualize script by Seoul Robotics, Direct Visualization
        '''
        if direct_use & txt_saved:

            visualize_lane_line = True  # Visualize lane line if available

            vis = Seoul_Robotics.Vis(self.data_folder, self.lane_folder, visualize_lane_line)
            vis.run()
    ...
    def peak_intensity_ratio(self, orig_points, indicator, plot_visual):
        ...
        if indicator == "front":
            ...
            points = self.SetROI(orig_points, self.ROI_hist_min, self.ROI_hist_max, None, None, None, None, None, None)
        
        elif indicator == "back":

            points = self.SetROI(orig_points, -self.ROI_hist_max, -self.ROI_hist_min, None, None, None, None, None, None)
    ...
    def save_lane_coeff_txt(self, left_coeff, right_coeff, is_save):
        '''
        Save coefficients of 3 order ploynomial with txt format
        '''
        txt = file_num + ".txt"
        ...
        if is_save:
            ...
            np.savetxt("./lane_folder/"+txt, lane_coeffi, fmt='%.18e', delimiter=';')
...
if __name__ == "__main__":
    ...
    file_num = "1553565729015329642"

    my_Lane = Lane(file_num)
